...
const DocumentsWidgetSmart: React.FC = () => {
  ...
  const checkConnection = async () => {
    try {
      ...
      if (error) {
        console.log('Database connection failed, using offline mode:', error.message);
        setIsOnline(false);
      ...
      console.log('Connection error, using offline mode:', error);
      setIsOnline(false);
      setDocuments(initialDocuments);
    } finally {
      setLoading(false);
  ...
  const loadDocuments = async () => {
    try {
      ...
      setDocuments(data || initialDocuments);
    } catch (error) {
      console.error('Error loading documents:', error);
      setDocuments(initialDocuments);
    }
  ...
  const uploadToSupabase = async () => {
    ...
    try {
      ...
      if (error) {
        ...
        throw new Error(`No se pudo guardar la información: ${error.message}`);
      }
      ...
      // Fallback a modo offline si hay error
      await uploadOffline();
      Alert.alert('Aviso', 'Documento guardado en modo offline. Se sincronizará cuando la conexión esté disponible.');
    }
  };
  ...
  const handleDownload = (doc: Document) => {
    if (Platform.OS === 'web') {
      if (isOnline && doc.file_url.startsWith('http')) {
          ...
          <View style={styles.modalContent}>
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>
                Subir Documento {!isOnline && '(Offline)'}
              </Text>
...
export default DocumentsWidgetSmart;